...
const Datasupserptions = () => {
 ...
 /* const actioncolumn =[ 
    ...
    {field:"daysNumber",headerName:"DaysNumber",width:120,renderCell:(params)=>{
  
        return(params.row.daysNumber)}},
    
    {field:"price",headerName:"Price",width:120,renderCell:(params)=>{
  
      return(params.row.price)}},
  
      {field:"name",headerName:"Type",width:120,renderCell:(params)=>{
  ...
  selector:row =>row.name
    ...
    },
   {
    ...
    cell:row =>(
      <div className='cellaction'>
      <Link to={`/admin/listsupsecrption/${row.id}`} style={{textDecoration:"none"}}>
  ...
  const customstyle={
    ...
    const fetchaxios= async (page) => {
     try{
      const response= await axios.get(`https://yaamen1.com/api/subscriptions?page=${page}&per_page=${perpage}`,{
   ...
   const handledelet = async (id,e)=>{
    ...
    e.preventDefault();
    try{
      const resp= await axios.delete(`https://yaamen1.com/api/subscriptions/${id}`,{
  ...
  return (
    ...
    <Link to={`/admin/listsupsecrption/addsup `}style={{textDecoration:"none"}}>
          <button className='addbot'>add</button>
          </Link>
          </div>
    {loading ? <DataTable className='gridss'  
...
export default Datasupserptions