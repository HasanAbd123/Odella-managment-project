...
const Datalostandfound= () => {
  ...
  selector:row =>row.user.firstName
    ...
    selector:row =>row.user.phoneNumber
      ...
      selector:row =>row.user.expiredSubscriptionDate
        },
  ...
  const customstyle={
    ...
    const fetchaxios= async (page) => {
     try{
      const response= await axios.get(`https://yaamen1.com/api/lost_found?page=${page}&per_page=${perpage}`,{
...
export default Datalostandfound