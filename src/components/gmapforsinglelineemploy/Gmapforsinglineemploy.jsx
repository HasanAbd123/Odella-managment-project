...
const Gmapforsinglineemploy = () => {
  ...
  const [posti,setpos]=useState(null)
   ...
   const{singlelinforemploy}=useParams();
 
  console.log("lnnn",ln)
  console.log("ltttt",lt)
  const fetchsinglepos= async () => {
    try{
     const response= await axios.get(`https://yaamen1.com/api/transportationLines/${singlelinforemploy}`,{
  ...
  <form >
             <div className='all'>
                ...
                <div className='inputbo'>
                    ...
                    <input type="text" name='first' value={singlepos.from?singlepos.from.name:'not'} />

                </div>
                <div className='inputbo'>
                    <lable for="firstname">نقطة النهاية</lable>
                    <input type="text" name='first' value={singlepos.to?singlepos.to.name:'not'} />
...
export default Gmapforsinglineemploy